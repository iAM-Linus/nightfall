...
local timer = require("lib.hump.timer") -- Make sure timer is required
    ...
    if self.unit and self.unit.getAbilityCooldown and self.unit.canUseAbility then
        for i, slot in ipairs(self.slots) do
            if slot and slot.id then
                -- Ensure the methods exist before calling them
                if self.unit.getAbilityCooldown then
                   slot.currentCooldown = self.unit:getAbilityCooldown(slot.id)
                else
                   slot.currentCooldown = 0 -- Default or handle error
                end
                if self.unit.canUseAbility then
                   slot.canUse = self.unit:canUseAbility(slot.id)
                else
                   slot.canUse = false -- Default or handle error
                end
            end
        end
    end
...
-- NEW: Check if mouse coordinates are over an ability slot
function AbilityPanel:checkHover(x, y)
    ...
    for i, slot in ipairs(self.slots) do
        ...
        if x >= slotX and x <= slotX + slotSize and
           y >= slotY and y <= slotY + slotSize then
            return slot.id -- Return the ability ID if hovered
        end
    end

    return nil -- Return nil if no slot is hovered
end

-- NEW: Get ability slot data by ID
function AbilityPanel:getAbilityById(abilityId)
    for _, slot in ipairs(self.slots) do
        if slot.id == abilityId then
            return slot
        end
    end
    return nil
...
-- Handle mouse movement (updates tooltip logic)
function AbilityPanel:mousemoved(x, y)
    if not self.visible then return end

    local hoveredId = self:checkHover(x, y)

    if hoveredId then
        local slotData = self:getAbilityById(hoveredId)
        if slotData then
            self.showTooltip = true
            self.tooltipAbility = slotData
            self.tooltipX = x
            self.tooltipY = y
        else
            self.showTooltip = false
...
function AbilityPanel:useSelectedAbility(target, x, y)
    ...
    -- Use the ability (ensure the useAbility method exists on the unit)
    local success = false
    if self.unit.useAbility then
        success = self.unit:useAbility(ability.id, target, x, y)
    else
        print("ERROR: Unit does not have useAbility method!")
    end
